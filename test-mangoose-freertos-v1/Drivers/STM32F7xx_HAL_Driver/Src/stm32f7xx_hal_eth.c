...
extern TIM_HandleTypeDef htim2; // MARRR
...
HAL_StatusTypeDef HAL_ETH_Transmit(ETH_HandleTypeDef *heth, ETH_TxPacketConfigTypeDef *pTxConfig, uint32_t Timeout)
{
  ...
  if (heth->gState == HAL_ETH_STATE_STARTED)
  {
    ...
    // MARRR: TODO: delete this
    uint32_t millis = HAL_GetTick();
    uint32_t ten_ns = __HAL_TIM_GET_COUNTER(&htim2);
    heth->RxDescList.TimeStamp.TimeStampHigh = millis;
    heth->RxDescList.TimeStamp.TimeStampLow = ten_ns;

	printf("HAL_ETH_Transmit\r\n");
...
HAL_StatusTypeDef HAL_ETH_Transmit_IT(ETH_HandleTypeDef *heth, ETH_TxPacketConfigTypeDef *pTxConfig)
{
  ...
  if (heth->gState == HAL_ETH_STATE_STARTED)
  {
    ...
    uint32_t ten_ns = __HAL_TIM_GET_COUNTER(&htim2);
    heth->RxDescList.TimeStamp.TimeStampHigh = millis;
    heth->RxDescList.TimeStamp.TimeStampLow = ten_ns*10;
